...
- No tiene Codigo Innecesario  (0.5)      = 0.0 => para que es la funcion IsPrime que no hace nada?
- Funciona y Cumple con el Objetivo (1.5) = 1.0 => PArcialmente nolo hace en la forma esperada
...
## Print Multiples (0.0)
- No tiene Errores Ni Warnings (0.5)      = 0.5
- No tiene Codigo Innecesario  (0.5)      = 0.0 => pude unir IsMultiple3 y IsMultiple5 en una sola de IsMultipleOf
- Funciona y Cumple con el Objetivo (1.5) = 1.4 => No elimina la ultima coma
...
- Cumple con el Codigo Limpio (1.5)       = 0.7
  ...
  - tiene notaciones innecesarias IntCodigo, strData) ?               = OK
  - Usan Sustantivos para Clases y Verbos para Metodos?               = OK
  - Una sola palabra por concepto?                                    = X
  - No usan combinaciones o juegos de palabras?                       = OK
  - No tiene contexto adicional o superfluo?                          = OK
  - Usan Datos del dominio, del negocio, problema o solucion ?        = OK
  - Cumplen con el Estandar de Pascal y Camel Case?                   = OK
